...
/* В основном меняется количество STATE связанное с write factors */ 
...
`define MAIN_FFT_CONTROL_START_WRITE_FACTORS_REG_ADDR_6_STATE   4'b1010     /* - запись коэффициентов для fft           */

`define MAIN_FFT_CONTROL_RUN_WRITE_CONTROL_REG_ADDR_STATE       4'b1011     /* - start                                  */
`define MAIN_FFT_CONTROL_RUN_READ_STATUS_REG_ADDR_STATE         4'b1100     /* - wait int_o ждем конец работы fft       */
`define MAIN_FFT_CONTROL_FINISH_STATE                           4'b1101     /* - закончили                              */
    ...
    always @(posedge i_clk or posedge i_rst) begin
        if (i_rst == 1'b1) begin
            ...
            case (o_state)
                ...
                `WRITE_REGISTER(
                    `MAIN_FFT_CONTROL_START_WRITE_FACTORS_REG_ADDR_5_STATE,
                    `MAIN_FFT_CONTROL_START_WRITE_FACTORS_REG_ADDR_6_STATE,
