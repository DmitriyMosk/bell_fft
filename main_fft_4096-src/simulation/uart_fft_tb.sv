...
localparam fft_size_byte = fft_size * 4 * 2;
...
function automatic void readFileMem (string nameFile, ref byte outBytes[]);
    ...
    string readAddr;
    byte readByte[4];
    while($fscanf(file, "%s %2h%2h%2h%2h\n", readAddr, readByte[3], readByte[2], readByte[1], readByte[0]) == 5) begin
        outBytes = {outBytes, readByte[0], readByte[1], readByte[2], readByte[3]};
    end
    $fclose(file);
    return;    
endfunction
...
module uart_fft_tb #()();
    ...
    string input_folder_name        = "sim_in";
    string output_folder_name_tag   = "sim_out_%0d";
    
    initial begin
        string __output_folder_name;
        string in_fpath; 
        string out_fpath;

        $sformat(__output_folder_name, output_folder_name_tag, fft_size);

        in_fpath  = {input_folder_name, "/", input_file_name};
        out_fpath = {__output_folder_name, "/", output_file_name};

        resetUart(uart);
        switchMode(`FFT, uart);

        readFileMem(in_fpath, bytes_in);
        testUart(bytes_in, bytes_out, uart);

        f = $fopen(out_fpath); 
        for (i = 0; i < bytes_out.size(); i = i + 4) begin
            $fwrite (f, "@%4X %X%X%X%X\n", i / 4, bytes_out[i+3], bytes_out[i+2], bytes_out[i+1], bytes_out[i+0]);
        end
        $fclose (f); 
        ...
        // testUart(bytes_in_2, bytes_out_2, uart);
        // f = $fopen(output_file_name_2); 
        // for (i = 0; i < bytes_out_2.size(); i = i + 4) begin
        //     $fwrite (f, "@%4X %X%X%X%X\n", i / 4, bytes_out_2[i+3], bytes_out_2[i+2], bytes_out_2[i+1], bytes_out_2[i+0]);
        // end
        // $fclose (f);          
        $stop; 
    end
