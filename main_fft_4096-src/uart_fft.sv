...
// Размерность FFT (должна быть степенью двойки)
...
module fft (
    ...
    assign write_last_it    = (write_it_word == `BEL_FFT_DWIDTH / size_byte - 1) & i_ready      ;
    assign read_last_it     = read_it_word  == `BEL_FFT_DWIDTH / size_byte - 1                  ;
    /* wire которые определяли конец последнего адреса в памяти                                 */
    assign write_last_addr  = write_addr    == fft_size * 2 - 1                                 ;
    assign read_last_addr   = read_addr     == fft_size * 2 - 1                                 ;
    /* Управление памятью                                                                       */
    ...
    assign ram_writedata    = write_data                                                        ;
